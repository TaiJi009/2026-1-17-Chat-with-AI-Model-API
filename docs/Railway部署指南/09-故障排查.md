...
**常见错误**:
- `npm: command not found` - 若为 monorepo，请检查是否已将该服务的 **Root Directory** 设置为 `backend`（或 `frontend`），而不是从仓库根目录构建；否则 Nixpacks 不会识别为 Node 项目，基础镜像中无 npm。
- `Railpack could not determine how to build` / `Script start.sh not found` - 若为 monorepo，请将该服务的 **Root Directory** 设置为 `backend`（或 `frontend`），否则 Railpack 在根目录无法识别单一应用（根目录无 package.json），无法确定如何构建；无需改代码或添加 start.sh。
...
**常见错误**:
- **现象**：本地执行 `railway run npm run migrate` 报错 `getaddrinfo ENOTFOUND postgres.railway.internal`。
- **原因**：`DATABASE_URL` 为 Railway 内网地址，仅可在 Railway 内网解析，本机无法解析该主机名。
- **处理**：在 Railway 控制台进入后端服务的 Shell，在该 Shell 内执行 `npm run migrate`；或使用数据库的**公网**连接字符串写入本机 `backend/.env` 的 `DATABASE_URL`，并在本机直接执行 `npm run migrate`（不要用 `railway run`）。
